...
```mermaid
flowchart TD
    START(["Запрос: «сделай задачу»"]) --> CFG["Стартовая конфигурация: TASK-ID, artifact_dir, approval_mode, репозитории"]
    CFG --> STATE{"Есть .workflow-state.json с resumable = true?"}
    STATE -- "да" --> CONT["Возобновить с current_stage"]
    CONT --> PA
    STATE -- "нет" --> PA

    PA["1 · Продуктовый анализ (grill-me) → proposal.md"]
    PA --> G1{"2 · Gate: proposal"}
    G1 -- "manual — подтверждение пользователя<br/>auto — validate-artifacts.sh + self-review" --> TP["3 · Техническое планирование → spec.md"]
    TP --> G2{"4 · Gate: spec"}
    G2 -- "аналогично этапу 2" --> IM["5 · Реализация: preflight, изменения, тесты"]
    IM --> VF["6 · Верификация: Acceptance Criteria, ручные проверки, self-review"]
    VF --> CR["7 · Code review: независимый ревьюер с чистым контекстом"]
    CR --> DL{"8 · Доставка: есть явное разрешение пользователя?"}
    DL -- "разрешено: commit → push → PR" --> RP["9 · Отчёт: validate_report.py → report.html"]
    RP --> RT["10 · Ретроспектива → retrospective.md"]
    RT --> DONE(["Workflow завершён"])

    TP -. "requirements_error" .-> PA
    IM -. "test_failed" .-> IM
    VF -. "test_failed" .-> IM
    CR -. "review_required" .-> IM
    IM -. "preflight_blocked" .-> WAIT["Остановка: показать проблему и ждать решения (await_user)"]
...
Сплошные стрелки — основной путь; пунктирные — коды ошибок из контракта `stage-results` и возвраты на предыдущие этапы. Ошибка `unknown` повторяет этап один раз; более трёх возвратов на один этап останавливают workflow. Возвраты при мультирепозиторийной доставке не откатывают уже успешные Git-действия.
...
После успешного создания PR этап `dev-task-reporting` локально создаёт `{artifact_dir}/{TASK-ID}/report.html`. Отчёт связывает бизнес-проблему, решения, технические изменения, проверки, риски, commits и PR, а также временную шкалу выполнения (`workflow_trace`) — полный журнал прохождения всех этапов с метками времени и длительностью.

`report-data.json` создаётся оркестратором инкрементально с первого этапа и заполняется по мере прохождения стадий, включая журнал `workflow_trace`. Это позволяет сохранить частичные данные даже при остановке workflow на блокере или ошибке. Финальный HTML-отчёт рендерится только после успешного создания PR. Публикация отчёта не выполняется.

Завершает workflow ретроспектива: оркестратор создаёт `{artifact_dir}/{TASK-ID}/retrospective.md` — наблюдаемые проблемы этапов, конкретные предложения по улучшению навыков и вердикт («Новых точек для улучшения не найдено», если проблем не было). Сокращённая ретроспектива заполняется и при остановке на блокере, чтобы причины остановки не терялись.

Рядом ведётся файл состояния `.workflow-state.json` — текущий этап, завершённые стадии и причина остановки. При обрыве сессии (закончились токены, закрылась вкладка, ошибка процесса) новый запуск `dev-task-workflow` для той же задачи предлагает возобновить выполнение с места остановки, не пересобирая параметры и не повторяя завершённые этапы.

Каждый этап завершается машиночитаемым результатом `{artifact_dir}/{TASK-ID}/stage-results/{stage}.json`: статус, резюме, артефакты, а при блокере — код ошибки и `suggested_next`. Оркестратор валидирует результат по схеме `references/stage-result-schema.md` и зеркалит его в `.workflow-state.json` и `workflow_trace`, не разбирая свободный текст сообщений. Это делает передачу между стадиями формальным контрактом, а не пересказом.
