...
Файл создаётся инкрементально: оркестратор формирует скелет перед началом первого этапа, заполняет поля по мере прохождения стадий, а на финальном этапе reporting проводит полную валидацию и рендеринг HTML. Поля, для которых данные ещё не собраны, заполняются плейсхолдерами `Не зафиксировано` или пустыми массивами.

## Содержание

* [Корневой объект](#корневой-объект)
* [Метаданные и резюме](#метаданные-и-резюме)
* [Журнал выполнения (workflow trace)](#журнал-выполнения-workflow-trace)
...
* `status` — `in_progress`, `blocked` или `pr_created`. `in_progress` — workflow ещё выполняется, `blocked` — workflow остановлен на блокере, `pr_created` — финальное состояние после доставки;
...
## Журнал выполнения (workflow trace)

`workflow_trace` — массив записей о прохождении этапов workflow. Каждая запись фиксирует факт входа на этап, его завершения, блокировки или ошибки. Заполняется оркестратором инкрементально после каждого этапа.
...
```json
{
  ...
  "duration_ms": 925000,
  "summary": "3 раунда grill-me, 7 вопросов, 2 допущения",
...
* `summary` — краткое описание результата этапа (опционально);
* `approval` — результат approval gate: `auto_approved` или `user_approved` (только для этапов `approval-proposal` и `approval-spec`).

Записи с `event=error` или `event=blocked` должны содержать `summary` с описанием проблемы. Записи с `event=started` не содержат `duration_ms` и `approval`.

При финальной валидации (`task.status == "pr_created"`) массив должен содержать хотя бы одну запись о завершении каждого обязательного этапа.
